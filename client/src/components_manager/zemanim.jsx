...
const Zemanim = () => {
    ...
    const fetchZemanim = async () => {
        try {
            const response = await fetch("https://api.sunrise-sunset.org/json?lat=31.749&lng=35.216&formatted=0");
    ...
    return (
        <div>
            ...
            {error && <p style={{ color: "red" }}>Error: {error}</p>} {/* מציג שגיאה אם יש */}
            {zemanim && (
                <ul>
                    <li><strong>זריחה:</strong> {new Date(zemanim.sunrise).toLocaleTimeString()}</li>
                    <li><strong>שקיעה:</strong> {new Date(zemanim.sunset).toLocaleTimeString()}</li>
                    <li><strong>חצות היום:</strong> {new Date(zemanim.solar_noon).toLocaleTimeString()}</li>
                    <li><strong>אורך היום:</strong> {new Date(zemanim.day_length * 1000).toISOString().substr(11, 8)}</li>
                </ul>
            )}
        </div>
    );
